...
The next advantage of AdGuard VPN for iOS is [TrustTunnel](https://trusttunnel.org/) — a modern protocol developed by us, that’s open-source and can be used by anyone. It is extremely difficult to detect compared to other VPN protocols, and it is stable even with a poor Internet connection. [Read more about the protocol](https://trusttunnel.org/).
...
The *VPN* mode uses [TrustTunnel](https://trusttunnel.org/) — a modern protocol that we developed, that’s open-source and can be used by anyone. It provides the best combination of speed and security. In this mode, AdGuard VPN will not work with [AdGuard Ad Blocker for iOS](https://adguard.com/adguard-ios/overview.html).
