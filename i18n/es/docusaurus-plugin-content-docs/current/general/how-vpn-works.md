...
AdGuard VPN utiliza [TrustTunnel](https://trusttunnel.org/) — un protocolo VPN moderno y de código abierto desarrollado por nosotros. One of its advantages is that the traffic transmitted using the AdGuard VPN protocol is difficult to distinguish from the regular traffic. The VPN tunnel looks like normal HTTPS traffic, making it extremely difficult to detect and block. In addition, it is based on the mechanisms of the modern HTTP/2 protocol, which ensures high connection speed.
...
- [_TrustTunnel_](https://trusttunnel.org/) — un protocolo VPN moderno, sigiloso y de código abierto desarrollado por AdGuard VPN. Funciona de forma estable incluso con una conexión a Internet lenta y se oculta como tráfico normal, lo que dificulta su seguimiento y bloqueo
