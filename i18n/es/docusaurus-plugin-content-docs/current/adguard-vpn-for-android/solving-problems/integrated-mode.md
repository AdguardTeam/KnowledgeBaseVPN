...
AdGuard VPN has the VPN operating mode enabled by default, which uses [its open-source protocol](https://trusttunnel.org/). Proporciona la mejor combinación de velocidad de conexión y seguridad. Sin embargo, este modo de funcionamiento no permite que AdGuard VPN y AdGuard Ad Blocker funcionen simultáneamente. The Integrated mode, in turn, uses a special internal protocol, which makes it possible for the two apps to work together.
