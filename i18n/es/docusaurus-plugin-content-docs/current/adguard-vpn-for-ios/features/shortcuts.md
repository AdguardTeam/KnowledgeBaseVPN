...
Shortcuts, a tool built into iOS, makes it easy to manage your connections without opening AdGuard VPN. You can place a _Connect_ or _Disconnect_ shortcut on your Home Screen, in Control Center, or a in a widget, or set up a Siri phrase to connect with your voice.
...
AdGuard VPN provides two ready-made shortcuts: _Connect_ and _Disconnect_. _Connect_ turns on the VPN and connects to your last used location. _Disconnect_ turns off the VPN. You’ll find both under _App Shortcuts_ in the Shortcuts app. Add either one to any of the three places listed below.
...
2. Tap _Add Action_, go to _Apps_ and select _AdGuard VPN_.
3. Choose _Manage VPN connection_ and on the following screen, tap _Add to Shortcut_, and _Turn VPN connection On_.
...
3. Tap _Add Widget_.
4. While in edit mode, tap the widget to open the configuration menu, set _Folder_ to _Custom_, and assign _Connect_ to one slot and _Disconnect_ to the other.
...
2. Add an If action. Inside it, add _Open App_ and set it to the app you want — for example, YouTube.
3. Under _Otherwise_, add three actions in order: _AdGuard VPN_ → _Turn VPN Connection On_, _Wait_ (set to 2 seconds), and _Open App_ (same app as above).
...
In the AdGuard VPN app, go to _General_ → _Shortcuts_ → _Set up Siri phrases_. You will then have two options: _Record a phrase to connect_ or _Record a phrase to disconnect_.
