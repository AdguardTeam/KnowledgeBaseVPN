...
AdGuard VPN has the VPN operating mode enabled by default, which uses [its open-source protocol](https://trusttunnel.org/). Fornece a melhor combinação de velocidade de conexão e segurança. No entanto, esse modo operacional não permite que o AdGuard VPN e o Bloqueador de anúncios do AdGuard funcionem simultaneamente. The Integrated mode, in turn, uses a special internal protocol, which makes it possible for the two apps to work together.
