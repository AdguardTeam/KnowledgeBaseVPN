...
AdGuard VPN has the VPN operating mode enabled by default, which uses [its open-source protocol](https://trusttunnel.org/). 它提供连接速度和安全性的最佳组合。 不过，本操作模式不允许 AdGuard VPN 和 AdGuard 广告拦截程序同时工作。 The Integrated mode, in turn, uses a special internal protocol, which makes it possible for the two apps to work together.
