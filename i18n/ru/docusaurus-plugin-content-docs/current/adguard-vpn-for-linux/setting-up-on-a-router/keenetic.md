...
1. Войдите в аккаунт
   ...
   Для роутеров Keenetic импортируйте SSL-сертификат и укажите папку для пользовательского каталога. По умолчанию он может храниться во временном каталоге, и вы можете потерять настройки после перезагрузки. Запускайте следующие команды перед каждым новым сеансом и перед использованием AdGuard VPN CLI:
