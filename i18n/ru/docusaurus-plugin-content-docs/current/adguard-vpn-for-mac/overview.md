...
One or more DNS servers can be set up here. Это полезно, когда вы не хотите полагаться на DNS-сервер по умолчанию, предоставленный вашим интернет-провайдером. Выберите нужный из списка популярных DNS-сервисов или добавьте собственный сервер вручную. Рекомендуем добавить AdGuard DNS, который не только шифрует ваш DNS-трафик, но и идентифицирует запросы к вредоносным сайтам и перенаправляет их в «чёрную дыру».
