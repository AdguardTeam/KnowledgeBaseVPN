...
Dall'inizio, abbiamo deciso di sviluppare e implementare un protocollo VPN interno invece di scegliere una soluzione predefinita: sarebbe stato troppo facile, inoltre, non eravamo soddisfatti dei protocolli esistenti. The result has become [TrustTunnel](https://trusttunnel.org/) — a modern, stealthy, and fast open-source VPN protocol developed by AdGuard VPN.
...
There are situations where you don’t need your VPN to be active: for instance, when you’re ordering food or using maps. Per questo motivo, AdGuard VPN ha una funzionalità speciale: le Esclusioni.

![Exclusions](https://cdn.adguard.com/content/blog/articles/adguard-vpn/exclusions-en.png)

Puoi attivare AdGuard VPN **selettivamente**: soltanto per certi siti web o app. This option is for those who use a VPN service only in specific situations: to enter a blocked website, use an app or watch a TV show that’s not available in their country, or not to expose their data when visiting some websites. Turning a VPN on selectively also helps save VPN traffic.
